...
describe('.value', function () {
   ...
   describe('.forEach', function () {
      ...
      it('should never call iterator when paginate has no children', async function () {
         const stopped = await paginate.value.forEach(database.ref('noValues'), 10, iteratorStub)
         assert.strictEqual(stopped, false)
         sinon.assert.notCalled(iteratorStub)
      })

      it('should call iterator once when paginate has a single child', async function () {
         const stopped = await paginate.value.forEach(database.ref('singleValue'), 10, iteratorStub)
         assert.strictEqual(stopped, false)
      ...
      it('should call iterator the same number of times as children when paginate has a many children', async function () {
         ...
         const stopped = await paginate.value.forEach(database.ref('manyValues'), 10, iteratorStub)
         assert.strictEqual(stopped, false)
      ...
      it('should call iterator in same correct order', async function () {
         const stopped = await paginate.value.forEach(database.ref('sameValues'), Object.keys(initialData.sameValues).length, iteratorStub)
         assert.strictEqual(stopped, false)
         sinon.assert.callCount(iteratorStub, Object.keys(initialData.sameValues).length)
         sameValueKeyOrder.forEach((k, idx) => {
      ...
      it('should call iterator twice when iteration is stopped after 2 children', async function () {
         ...
         const stopped = await paginate.value.forEach(database.ref('sameValues'), 10, iteratorStub)
